...
func TestFeatureGateFlag(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(test.arg, func(t *testing.T) {
			...
			f.SetCompatibilityVersion("1.29")
			f.Add(map[Feature]VersionedSpecs{
				testAlphaGate: {{Version: mustParseVersion("1.29"), Default: false, PreRelease: Alpha}},
				testBetaGate:  {{Version: mustParseVersion("1.29"), Default: false, PreRelease: Beta}},
			})
			f.AddFlag(fs)
...
`
	...
	f := NewFeatureGateForTest("1.29")
	f.SetCompatibilityVersion("1.29")
	fMap := map[Feature]VersionedSpecs{
		testAlphaGate:    {{Version: mustParseVersion("1.29"), Default: false, PreRelease: Alpha}},
		testAlphaEnabled: {{Version: mustParseVersion("1.29"), Default: false, PreRelease: Alpha}},
		testBetaGate:     {{Version: mustParseVersion("1.29"), Default: true, PreRelease: Beta}},
		testBetaDisabled: {{Version: mustParseVersion("1.29"), Default: true, PreRelease: Alpha}},
	}
	f.Add(fMap)
...
func TestFeatureGateString(t *testing.T) {
	...
	featuremap := map[Feature]VersionedSpecs{
		testGAGate:    {{Version: mustParseVersion("1.29"), Default: true, PreRelease: GA}},
		testAlphaGate: {{Version: mustParseVersion("1.29"), Default: false, PreRelease: Alpha}},
		testBetaGate:  {{Version: mustParseVersion("1.29"), Default: true, PreRelease: Beta}},
	}
